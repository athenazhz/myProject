...
<script type="text/javascript">
	...
	function changeColor(obj, colorId) {
		...
		gColorId = colorId;
		//清除之前选择的颜色
		$("#colors a").attr("class", "changeToWhite");
		//清除显示的尺码
	...
	function changeSize(obj, size) {
		if($(obj).attr("class") == "changeToWhite") {
			//清空之前选择的尺码
			$("#sizes a").each(function() {
				if ($(this).attr("class") == "changeToRed") {
			...
			//确定显示价格
			<c:forEach items="${skuList}" var="sku">
				if (gColorId == '${sku.colorId}' && size == '${sku.size}') {
					//显示价格
					$("#pprice").html("￥${sku.price}");
					//显示市场价格
					$("#pmarketprice").html("￥${sku.marketPrice}");
	...
	//购买按钮
	function buy() {
		window.location.href = "/buy/shopping.html?skuId="+gskuId+"&amount="+$("#num").val()+"&url="+encodeURIComponent(window.location.href);
	}
</script>
</head>
<body>
...
<div class="w ofc mt">
	<div class="l">
		<div class="showPro">
			<div class="big"><a id="showImg" class="cloud-zoom" href="${product.img.imgUrl }" rel="adjustX:10,adjustY:-1"><img alt="" src="${product.img.imgUrl }"></a></div>
		</div>
	</div>
	<div class="r" style="width: 640px">
		<ul class="uls form">
			<li><h2>${product.name }</h2></li>
			<li><label>价 格：</label><span class="word"><b class="f14 red mr" id="pprice" >￥</b>(市场价:<del id="pmarketprice" >￥</del>)</span></li>
			<li><label>商品评价：</label><span class="word"><span class="val_no val3d4" title="4分">4分</span><var class="blue">(已有888人评价)</var></span></li>
			<li><label>运　　费：</label><span class="word" id="pdeliveFee" ></span></li>
			<li><label>库　　存：</label><span class="word" id="pstock"></span><span class="word" >件</span></li>
			<li><label>选择颜色：</label>
				<div id="colors" class="pre spec">
					<c:forEach items="${colorList}" var="color" >
						<a onclick="changeColor(this,'${color.id}')" href="javascript:void(0)" title="${color.name }" class="changeToWhite"><img width="25" height="25" data-img="1" src="/jsutils/img/pic/ppp00.jpg" alt="${color.name } "><i>${color.name }</i></a>
					</c:forEach>
					
				</div>
			</li>
			<li id="sizes"><label>尺　　码：</label>
						<a href="javascript:void(0)" class="not-allow" onclick="changeSize(this,'S');" id="S">S</a>
						<a href="javascript:void(0)" class="not-allow" onclick="changeSize(this,'M');" id="M">M</a>
						<a href="javascript:void(0)" class="not-allow" onclick="changeSize(this,'L');" id="L">L</a>
						<a href="javascript:void(0)" class="not-allow" onclick="changeSize(this,'XL');" id="XL">XL</a>
						<a href="javascript:void(0)" class="not-allow" onclick="changeSize(this,'XXL');" id="XXL">XXL</a>
			</li>
