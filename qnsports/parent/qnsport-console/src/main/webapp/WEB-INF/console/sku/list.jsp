...
<title>qingniao-list</title>
	<script type="text/javascript">
		...
		function addSku(skuId) {
			var marketPrice = $("#m"+skuId).attr("disabled",true).val();
			var price = $("#p"+skuId).attr("disabled",true).val();
			var stock = $("#s"+skuId).attr("disabled",true).val();
			var upperLimit = $("#u"+skuId).attr("disabled",true).val();
			//使用 jquery异步提交
			$.ajax({
				type : 'post',
				data : {"marketPrice":marketPrice, "price":price, "stock":stock, "upperLimit":upperLimit, "id":skuId},
				url : "/sku/save.do",
				dataType : 'json',
