...
function delProduct(skuId) {
	alert(skuId);
	window.location.href="/delete/userItem.html?skuId="+skuId;
}
...
function addProductAmount(skuId, upperLimit) {
	...
	$("#num"+skuId).val(num);
	//ajax异步提交
	var url = "/userItem/addAmount.html";
	var params = {
			"skuId" : skuId,
			"amount" : 1,
	};
	$.post(url, params, function(data) {
		...
		//商品数量
		$("#productAmount").html(data.productsAmount);
	});
}

//减少商品购买数量
function desProductAmount(skuId) {
	var num = $("#num"+skuId).val();
	num--;
	if (num == 0) {
		alert("不能小于零");
...
<c:if test="${fn:length(userCart.userItems)>0 }">
<div class="w ofc case">
	<div class="confirm">
		...
		<div class="ofc pb40">
			...
			<tbody>
			<c:forEach items="${userCart.userItems }" var="item" >
				<tr>
					<td class="nwp pic">
						<ul class="uls">
							<li>
								<a class="pic" title="#" href="#"><img alt="#" src="${item.sku.product.img.imgUrl}"></a>
								<dl>
									<dt><a title="#" href="#"> ${item.sku.product.name }--${item.sku.color.name }--${item.sku.size }</a></dt>
					...
					<td>
						<a onclick="desProductAmount('${item.sku.id}')" class="inb arr" title="减" href="javascript:void(0);">-</a>
			...
			<div class="page">
				...
				<span class="r box_gray">
					<dl class="total">
						<dt>购物车金额小计：<cite>(共<var id="productAmount">${userCart.productsAmount }</var>个商品)</cite></dt>
						<dd><em class="l">商品金额：</em>￥<var id="price">${userCart.price }</var>元</dd>
						<dd><em class="l">运费：</em>￥<var id="extra">${userCart.extra }</var></dd>
						<dd class="orange"><em class="l">应付总额：</em>￥<var id="allPrice">${userCart.totalPrice }</var>元</dd>
